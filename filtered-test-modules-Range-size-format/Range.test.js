...
describe('range', function() {
  ...
  describe('size', function () {
    it('should calculate the size of a range', function() {
      ...
      let recObj0 = new Range(0, 0);
      assert.deepEqual(recObj0.size(), [0]);

      let recObj1 = new Range(0, 0, -1);
      assert.deepEqual(recObj1.size(), [0]);

      let recObj2 = new Range(0, 0, 0);
      assert.deepEqual(recObj2.size(), [0]);

      let recObj3 = new Range(0, 4);
      assert.deepEqual(recObj3.size(), [4]);

      let recObj4 = new Range(2, 4);
      assert.deepEqual(recObj4.size(), [2]);

      let recObj5 = new Range(0, 8, 2);
      assert.deepEqual(recObj5.size(), [4]);

      let recObj6 = new Range(0, 8.1, 2);
      assert.deepEqual(recObj6.size(), [5]);

      let recObj7 = new Range(0, 7.9, 2);
      assert.deepEqual(recObj7.size(), [4]);

      let recObj8 = new Range(0, 7, 2);
      assert.deepEqual(recObj8.size(), [4]);

      let recObj9 = new Range(3, -1, -1);
      assert.deepEqual(recObj9.size(), [4]);

      let recObj10 = new Range(3, -1.1, -1);
      assert.deepEqual(recObj10.size(), [5]);

      let recObj11 = new Range(3, -0.9, -1);
      assert.deepEqual(recObj11.size(), [4]);

      let recObj12 = new Range(3, -1, -2);
      assert.deepEqual(recObj12.size(), [2]);

      let recObj13 = new Range(3, -0.9, -2);
      assert.deepEqual(recObj13.size(), [2]);

      let recObj14 = new Range(3, -1.1, -2);
      assert.deepEqual(recObj14.size(), [3]);

      let recObj15 = new Range(3, 0.1, -2);
      assert.deepEqual(recObj15.size(), [2]);
    });
  });
  ...
  describe('toString', function () {
    ...
    it('should stringify a range to format start:step:end with given precision', function () {
      ...
      let recObj0 = new math.type.Range(1/3, 4/3, 2/3);
      assert.equal(recObj0.format(3), '0.333:0.667:1.33');

      let recObj1 = new math.type.Range(1/3, 4/3, 2/3);
      assert.equal(recObj1.format(4), '0.3333:0.6667:1.333');

      let recObj2 = new math.type.Range(1/3, 4/3, 2/3);
      assert.equal(recObj2.format(), '0.3333333333333333:0.6666666666666666:1.3333333333333333');
    });
  });
  ...
  describe('format', function () {
    it('should format a range as string', function () {
      ...
      let recObj0 = new Range(0, 4);
      assert.equal(recObj0.format(), '0:4');

      let recObj1 = new Range(0, 4, 2);
      assert.equal(recObj1.format(), '0:2:4');

      let recObj2 = new Range(0.01, 0.09, 0.02);
      assert.equal(recObj2.format(), '0.01:0.02:0.09');

      let recObj3 = new Range(0.01, 0.09, 0.02);
      let arg30 = {
        notation: 'exponential'
      };

      assert.equal(recObj3.format(arg30), '1e-2:2e-2:9e-2');
    });
  });
