...
var approx = require('../tools/approx.js');
...
describe('OperatorNode', function() {
  ...
  describe('toString', function () {
    ...
    it ('should stringify left associative OperatorNodes that are associative with another Node', function () {
      ...
      let invArg0 = {parenthesis: 'auto'};
      let recObj0 = math.parse('(a+b)+c');
      assert.equal(recObj0.toString(invArg0), 'a + b + c');

      let recObj1 = math.parse('a+(b+c)');
      assert.equal(recObj1.toString(invArg0), 'a + b + c');
      
      let recObj2 = math.parse('(a+b)-c');
      assert.equal(recObj2.toString(invArg0), 'a + b - c');

      let recObj3 = math.parse('a+(b-c)');
      assert.equal(recObj3.toString(invArg0), 'a + b - c');

      let recObj4 = math.parse('(a*b)*c');
      assert.equal(recObj4.toString(invArg0), 'a * b * c');
      
      let recObj5 = math.parse('a*(b*c)');
      assert.equal(recObj5.toString(invArg0), 'a * b * c');

      let recObj6 = math.parse('(a*b)/c');
      assert.equal(recObj6.toString(invArg0), 'a * b / c');

      let recObj7 = math.parse('a*(b/c)');
      assert.equal(recObj7.toString(invArg0), 'a * b / c');
