...
describe('Complex', function () {
  ...
  describe('format', function() {
    ...
    it('should format a complex number', function() {
      ...
      let recObj0 = new Complex(2, 3);
      assert.equal(recObj0.format(), '2 + 3i');

      let recObj1 = new Complex(2, -3);
      assert.equal(recObj1.format(), '2 - 3i');

      let recObj2 = new Complex(-2, 3);
      assert.equal(recObj2.format(), '-2 + 3i');

      let recObj3 = new Complex(-2, -3);
      assert.equal(recObj3.format(), '-2 - 3i');

      let recObj4 = new Complex(2, 1);
      assert.equal(recObj4.format(), '2 + i');

      let recObj5 = new Complex(2, -1);
      assert.equal(recObj5.format(), '2 - i');

      let recObj6 = new Complex(2, 0);
      assert.equal(recObj6.format(), '2');

      let recObj7 = new Complex(0, 2);
      assert.equal(recObj7.format(), '2i');
    });

    it('should format a complex number with custom precision', function() {

      let recObj0 = new Complex(1/3, 1/3);
      assert.equal(recObj0.format(3), '0.333 + 0.333i');

      let recObj1 = new Complex(1/3, 1/3);
      assert.equal(recObj1.format(4), '0.3333 + 0.3333i');

      let recObj2 = new Complex(1/3, 1/3);
      assert.equal(recObj2.format(), '0.3333333333333333 + 0.3333333333333333i');
    });

    it('should round im to zero if very small compared to re', function() {

      let recObj0 = new Complex(-1, 1.22e-16);
      assert.equal(recObj0.format(), '-1 + 1.22e-16i');

      let recObj1 = new Complex(-1, 1.22e-16);
      assert.equal(recObj1.format(15), '-1');

      let recObj2 = new Complex(-1, -1.22e-16);
      assert.equal(recObj2.format(15), '-1');

      let recObj3 = new Complex(1, -1.22e-16);
      assert.equal(recObj3.format(15), '1');

      let recObj4 = new Complex(1, 1.22e-16);
      assert.equal(recObj4.format(15), '1');

      let recObj5 = new Complex(-1, 1e-7);
      assert.equal(recObj5.format(5), '-1');
    });

    it('should round re to zero if very small compared to im', function() {

      let recObj0 = new Complex(1.22e-16, -1);
      assert.equal(recObj0.format(), '1.22e-16 - i');

      let recObj1 = new Complex(1.22e-16, -1);
      assert.equal(recObj1.format(15), '-i');

      let recObj2 = new Complex(-1.22e-16, -1);
      assert.equal(recObj2.format(15), '-i');

      let recObj3 = new Complex(-1.22e-16, 1);
      assert.equal(recObj3.format(15), 'i');

      let recObj4 = new Complex(1.22e-16, 1);
      assert.equal(recObj4.format(15), 'i');

      let recObj5 = new Complex(1e-7, -1);
      assert.equal(recObj5.format(5), '-i');
    });
  });
  ...
  describe('fromPolar', function() {
    it('should save polar coordinates input correctly', function() {
      ...
      let arg00 = {r: 0, phi: 4};
      var complex1 = Complex.fromPolar(arg00);

      let arg10 = {r: 5, phi: 0};
      var complex2 = Complex.fromPolar(arg10);

      let arg20 = {r: 1, phi: Math.PI};
      var complex3 = Complex.fromPolar(arg20);

      let arg30 = {r: 3, phi: Math.PI / 2};
      var complex4 = Complex.fromPolar(arg30);

      let arg40 = {r: 3, phi: -Math.PI / 2};
      var complex5 = Complex.fromPolar(arg40);
    ...
    it('should have the same value for the different import ways', function() {
      ...
      let arg00 = {r: 1, phi: 1};
      var way2 = Complex.fromPolar(arg00);
      assert(way1.equals(way2));
    });

    it('should accept angle units for phi properly', function() {

      let arg01 = new Unit(90, 'deg');
      var fromDeg = Complex.fromPolar(1, arg01);

      let arg11 = new Unit(0, 'rad');
      var fromRad = Complex.fromPolar(1, arg11);

      let arg21 = new Unit(100, 'grad');
      var fromGrad = Complex.fromPolar(1, arg21);
